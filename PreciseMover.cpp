...
PreciseMover::PreciseMover(
    ...
    TARGET_TWIST_OMEGA = newTargetTwistOmega;
    ...
    pidTW.construct(&inputTW, &outputTW, &setpointTW, kpTW, kiTW, kdTW, DIRECT);
    pidTW.SetSampleTime(PID_SAMPLE_TIME);
    pidTW.SetOutputLimits(PID_MIN_TW, PID_MAX_TW);
    pidTW.SetMode(AUTOMATIC);
...
void PreciseMover::twist(double angle) {
    ...
    while (absDouble(angleError) > ANGLE_ERROR_THRESHOLD) {
        ...
        if (millis() - prevCorrectionTime > CORRECTION_TIME_INTERVAL_TW) {
            ...
            //input_TW = abs(angleError);
            inputTW = absDouble(deadReckoner.getW() * 30 / PI); // in RPM
            if (angleError > 0) {
                motorSetCCW();
            ...
            Serial.print("\toutputTW: "); Serial.println(outputTW);
        }
        pidTW.Compute();
